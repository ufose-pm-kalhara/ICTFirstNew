  ...
  try {
    ...
    if (status === 'completed' || status === 'approved') {
      await createNotification(
        targetStudentId,
        "Payment Approved ✅",
        `Your payment of Rs. ${amount} for ${month} has been verified successfully.\nඔබගේ ${month} මාසයට අදාළ රු. ${amount} ගෙවීම සාර්ථකව තහවුරු කර ඇත.`
      );
    } else if (status === 'rejected') {
      await createNotification(
        targetStudentId,
        "Payment Rejected ❌",
        `Your payment for ${month} was rejected. Reason: ${remarks || 'Invalid proof'}. Please re-upload.\nඔබගේ ${month} මාසයට අදාළ ගෙවීම ප්‍රතික්ෂේප කර ඇත. හේතුව: ${remarks || 'අසත්‍ය සාක්ෂි'}. කරුණාකර නැවත උඩුගත කරන්න.`
      );
    }
